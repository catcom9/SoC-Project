...
For the project we received a project template. This template included all the files needed to display colour strips on the monitor. The template included the following
 - VGATop.v
 ...
 - VGAColourCycle.v
    - Generates the colour data for displaying on the screen. As the name suggests it cycles through a few colours.
 - Basys3_Master.xdc
...
I researched how to load large amounts of data into a verilog file and found the `$readmemh` function this allowed the reading in of a file into memory. The blog [ProjectF](https://projectf.io/posts/initialize-memory-in-verilog/) is where I found most of this information.



But first I needed a file to load. So I created a program to create a memory file filled with one colour value. Then manually changed a few values to a different colour. This worked and produced the following image.
...
The image need to be in the correct format for the `$readmemh` function. Each pixel value needed to be in hexadecimal and a white space character in between.
...
### **Running Out of Room**
When changing out the image from the colour fill to anything else. It was unable to synthesize the project. I ran out of LUTs (Look up tables) to use. This was strange as the design worked with the colour fill image but not anything else. It seems that it was optimizing out most of the data from the colour fill image but it couldn't do this for any other as they are more complex images using more colours.
...
After changing the code to use the pallet and fixing how the address was calculated it worked as intended. Below is the final output.
...
<img src="docs/assets/images/RTL_Nobram.png">
...
<img src="docs/assets/images/RTL_BRAM.png">
